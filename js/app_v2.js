...
window.saveToCloud = async () => {
    ...
    try {
        ...
        // alert("?¥Îùº?∞Îìú ?Ä???ÑÎ£å"); // Too spammy if auto save? 
        // User asked for "Save to Cloud" button.
    } catch (e) {
        console.error("Cloud Save Error:", e);
        alert(`?¥Îùº?∞Îìú ?Ä???§Ìå®: ${e.message}`);
    }
};
    ...
    try {
        ...
        if (btnUseDefaults) {
            btnUseDefaults.addEventListener('click', () => {
                ...
                alert("?ÑÏû¨ ?§Ï†ï(?úÎìú ?¨Ìï®)??'Îß§Îß§ ?úÌä∏' Í∏∞Î≥∏Í∞íÏúºÎ°??Ä?•Îêò?àÏäµ?àÎã§.");
        ...
        const openInjModal = (type) => {
            currentInjType = type;
            document.getElementById('injModalTitle').textContent = type === 'SEED' ? '?úÎìú ?êÍ∏à Î≥Ä???†Ï≤≠' : '?àÏàòÍ∏?Î≥Ä???†Ï≤≠';
            ...
            const dateLabel = dateInput.previousElementSibling; // The label "?ÅÏö© ?¨Îßù ?†Ïßú"
            ...
            if (type === 'SEED') {
                ...
                if (dateMsg) {
                    ...
                    dateMsg.innerHTML = `?îÑ ?§Ïùå ?êÍ∏à Í∞±Ïã†(Î¶¨Î∞∏?∞Ïã±)?ºÏù∏<br><span style="color:#059669; font-size:1.05rem;">${nextDate}</span> ??br>?êÎèô?ºÎ°ú ?©ÏÇ∞?òÏñ¥ Î∞òÏòÅ?©Îãà??`;
                ...
                if (dateMsg) {
                    dateMsg.innerHTML = `??<b>Ï¶âÏãú Î∞òÏòÅ</b><br>?§Îäò(${nextBizDay}) ?†ÏßúÎ°??àÏàòÍ∏àÏóê Î∞îÎ°ú Î∞òÏòÅ?©Îãà??`;
                    dateMsg.classList.remove('hidden');
                }
            }
        ...
        if (btnSaveInj) {
            btnSaveInj.addEventListener('click', () => {
                ...
                if (currentInjType === 'SEED') {
                    date = document.getElementById('injDate').dataset.autoDate || "?§Ïùå Í∞±Ïã†??;
                } else {
                    date = document.getElementById('injDate').value;
                    if (!date) { alert("?†ÏßúÎ•??ÖÎ†•?¥Ï£º?∏Ïöî."); return; }
                }

                if (!amt) { alert("Í∏àÏï°???ÖÎ†•?¥Ï£º?∏Ïöî."); return; }
        ...
        window.renderInjHistory = () => {
            ...
            if (history.length === 0) {
                list.innerHTML = "<li style='color:#999;'>?¥Ïó≠???ÜÏäµ?àÎã§.</li>";
                return;
            }

            list.innerHTML = history.slice().reverse().map(item => { // Show newest first
                const typeLabel = item.type === 'SEED' ? '<span style="color:#10b981;">[?úÎìú]</span>' : '<span style="color:#2563eb;">[?àÏàòÍ∏?</span>';
                ...
                <div>
                   ...
                   <div style="font-size:0.8rem; color:#666;">?¨Îßù?? ${dateLabel}</div>
                </div>
                <button onclick="deleteInjection(${item.id})" style="background:#ef4444; color:white; border:none; padding:4px 8px; border-radius:4px; cursor:pointer;">??†ú</button>
            </li>`;
            }).join('');
        };
        ...
        const saveDefaults = () => {
            ...
            alert("?ÑÏû¨ ?§Ï†ï(?úÎìú ?¨Ìï®, ?úÏûë???¨Ìï®)??'Í∏∞Î≥∏Í∞??ºÎ°ú ?Ä?•Îêò?àÏäµ?àÎã§.");
        ...
        if (btnSaveSeed) {
            btnSaveSeed.addEventListener('click', () => {
                ...
                localStorage.setItem('userSeed', val);
                alert("Ï¥àÍ∏∞ ?úÎìúÍ∞Ä ?Ä?•Îêò?àÏäµ?àÎã§: $" + val);
            });
        }
    } catch (criticalError) {
        console.error("CRITICAL APP ERROR:", criticalError);
        alert("?†ÌîåÎ¶¨Ï??¥ÏÖò Ï¥àÍ∏∞??Ï§?ÏπòÎ™Ö???§Î•ò Î∞úÏÉù:\n" + criticalError.message);
    }
});
...
function updateTierInputs() {
    ...
    if (safe8) {
        ...
        safe8.title = isRealTier ? "Real Tier Î™®Îìú?êÏÑú??8Ï∞®Ïàò(Î¨¥ÌïúÎß§Ïàò)Í∞Ä ÎπÑÌôú?±Ìôî?©Îãà??" : "";
    }
    if (off8) {
        off8.disabled = isRealTier;
        off8.style.backgroundColor = isRealTier ? '#e5e7eb' : '';
        off8.style.opacity = isRealTier ? "0.3" : "1";
        off8.title = isRealTier ? "Real Tier Î™®Îìú?êÏÑú??8Ï∞®Ïàò(Î¨¥ÌïúÎß§Ïàò)Í∞Ä ÎπÑÌôú?±Ìôî?©Îãà??" : "";
    }
...
document.getElementById('btnStartDeepMind').addEventListener('click', () => {
    ...
    document.getElementById('dmLoadingText').textContent = "AIÍ∞Ä ?§Ï†ï??Î≤îÏúÑ ?¥Ïóê??ÏµúÏ†ÅÍ∞íÏùÑ ?êÏÉâ Ï§ëÏûÖ?àÎã§...";
...
window.openRobustnessModal = async (idx) => {
    ...
    pEl.innerHTML = `
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 1rem; text-align: left;">
            <div>
                <div style="color: #fbbf24; font-weight: bold; margin-bottom: 0.5rem; text-transform: uppercase;">?õ°Ô∏?Safe Mode</div>
            ...
            <div>
                <div style="color: #f472b6; font-weight: bold; margin-bottom: 0.5rem; text-transform: uppercase;">?îÔ∏è Offensive Mode</div>
...
function renderTable(result) {
    ...
    if (ledger.length === 0) {
        ...
        tbody.innerHTML = "<tr><td colspan='20' style='text-align:center; padding:20px; color:#64748b;'>?∞Ïù¥?∞Í? ?ÜÏäµ?àÎã§ (Í∏∞Í∞Ñ/?§Ï†ï ?ïÏù∏ ?ÑÏöî)</td></tr>";
        return;
    }
    ...
    if (state && params) {
        ...
        const nextDayKo = ['??, '??, '??, '??, 'Î™?, 'Í∏?, '??][nextDate.getDay()];
        ...
        const modeKo = modeName === "Safe" ? "?àÏ†Ñ" : "Í≥µÏÑ∏";

        html += `
        <tr style="background-color: #fefce8; border-bottom: 2px solid #fbbf24;">
            <td style="font-weight:bold; color:#1e1e1e; text-align: center;">${nextDateStr} (${nextDayKo})<br><span style="font-size:0.8em; color:#d97706;">(?àÏÉÅ)</span></td>
            <td>-</td>
    ...
    ledger.forEach(row => {
        ...
        const modeKo = row.mode === "Safe" ? "?àÏ†Ñ" : "Í≥µÏÑ∏";
...
function renderOrderListHTML(orders) {
    if (orders.length === 0) return '<div style="color:#1e1e1e; font-weight:bold; text-align:center; padding:10px;">?§Îäò Ï£ºÎ¨∏???ÜÏäµ?àÎã§</div>';

    return orders.map((o, i) => {
        const isBuy = o.type.includes('buy') || o.type.includes('Îß§Ïàò') || o.isBuy;
        const color = isBuy ? '#ef4444' : '#2563eb';

        // Text Construction
        // o.text usually contains the full string "LOC Îß§Ïàò 10Í∞?@ $100"
        let displayType = o.text ? o.text : "";

        if (!displayType) {
            const typeLabel = o.type === 'MOC' ? 'MOC Îß§ÎèÑ' : (isBuy ? 'LOC Îß§Ïàò' : 'LOC Îß§ÎèÑ');
            const qtyLabel = `${o.qty}Í∞?;
...
window.adjustTargetAndRender = () => {
    ...
    if (mode === 'Offensive') {
        if (locSells.length >= 2) {
            ...
            // LOC Sells <= 1
            const activeSells = d.sells.filter(s => s.qty > 0); // Include MOC here? User said "Î™©ÌëúÎß§ÎèÑÍ∞Ä loc...". But visual applies to all?
            // "ÎßåÏïΩ Í≥µÏÑ∏Î™®Îìú?¥Î©¥??Î™©ÌëúÎß§ÎèÑÍ∞Ä locÎß§ÎèÑÏ£ºÎ¨∏??1Í∞úÏù¥??mocÎß§ÎèÑÏ£ºÎ¨∏?Ä Î™©ÌëúÎß§ÎèÑÍ∞Ä locÎß§ÎèÑÏ£ºÎ¨∏?ºÎ°ú Ïπ¥Ïö¥???àÌï®)??Í≤ΩÏö∞"
            // "Îß§ÎèÑ Í∞?àòÍ∞Ä 0Í∞úÏù∏ Ï£ºÎ¨∏Îß??àÍ±∞???ÑÎ¨¥ Îß§ÎèÑ Ï£ºÎ¨∏???ÜÎäî Í≤ΩÏö∞??'?§Îäò Ï£ºÎ¨∏???ÜÏäµ?àÎã§'"

            // Check if ANY active SELL (qty>0). 
            if (activeSells.length > 0) {
                // "Í∑?Ï£ºÎ¨∏Îß??åÎ??âÏúºÎ°?.. ?ÅÏπòÍ∏?Î≤ÑÌäº?Ä ?àÎ≥¥?¨Ï§ò????"
                showNetting = false;
                activeOrders = activeSells;
            } else {
                message = "?§Îäò Ï£ºÎ¨∏???ÜÏäµ?àÎã§";
                showNetting = false;
            }
        }
    } else {
        ...
        if (locSells.length >= 1) {
            ...
            // "Î™©Ìëú Îß§ÎèÑÍ∞Ä locÏ£ºÎ¨∏???ÜÏúºÎ©?'?§Îäò Ï£ºÎ¨∏???ÜÏäµ?àÎã§'"
            message = "?§Îäò Ï£ºÎ¨∏???ÜÏäµ?àÎã§";
            showNetting = false;
        }
    }
...
window.calculateAndRenderNetting = () => {
    ...
    if (buyPrice < lowestLoc) {
        if (buyQty > 0) finalOrders.push({ type: 'buy', text: `LOC Îß§Ïàò ${buyQty}Í∞?@ $${buyPrice.toFixed(2)}`, price: buyPrice });
        mocSells.forEach(s => {
            // MOC is effectively sell w/ price 0 (for netting logic doesn't matter, just separate type)
            if (s.qty > 0) finalOrders.push({ type: 'sell_moc', text: `MOC Îß§ÎèÑ ${s.qty}Í∞?, price: 0 });
        });
        locSells.forEach(s => {
            if (s.qty > 0) finalOrders.push({ type: 'sell_loc', text: `LOC Îß§ÎèÑ ${s.qty}Í∞?@ $${s.price.toFixed(2)}`, price: s.price });
        });
    } else {
        ...
        locSells.forEach(s => {
            if (s.qty === 0) return;
            if (s.price > buyPrice) {
                finalOrders.push({ type: 'sell_loc', text: `LOC Îß§ÎèÑ ${s.qty}Í∞?@ $${s.price.toFixed(2)}`, price: s.price });
            } else {
                if (currentTarget >= s.qty) {
                    if (s.qty > 0) finalOrders.push({ type: 'buy', text: `LOC Îß§Ïàò ${s.qty}Í∞?@ $${(s.price - 0.01).toFixed(2)}`, price: s.price - 0.01 });
                    currentTarget -= s.qty;
                } else {
                    if (currentTarget > 0) {
                        finalOrders.push({ type: 'buy', text: `LOC Îß§Ïàò ${currentTarget}Í∞?@ $${(s.price - 0.01).toFixed(2)}`, price: s.price - 0.01 });
                    }
                    const remSell = s.qty - currentTarget;
                    finalOrders.push({ type: 'sell_loc', text: `LOC Îß§ÎèÑ ${remSell}Í∞?@ $${s.price.toFixed(2)}`, price: s.price });
                    currentTarget = 0;
                }
            }
        });
        ...
        if (currentTarget > 0) {
            finalOrders.push({ type: 'buy', text: `LOC Îß§Ïàò ${currentTarget}Í∞?@ $${buyPrice.toFixed(2)}`, price: buyPrice });
        }
        ...
        finalOrders.forEach(o => {
            if (o.type.includes('sell')) {
                if (o.price <= buyPrice) {
                    const match = o.text.match(/\d+Í∞?);
                    if (match) activeSells += parseInt(match[0].replace('Í∞?, ''));
                }
            }
        });
        ...
        if (needed > 0) {
            finalOrders.push({ type: 'sell_loc', text: `LOC Îß§ÎèÑ ${needed}Í∞?@ $${(buyPrice + 0.01).toFixed(2)}`, price: buyPrice + 0.01 });
        }
    }
    ...
    resArea.innerHTML = `<h4 style="margin:0 0 10px 0; color:#d97706;">??ÏµúÏ¢Ö Ï£ºÎ¨∏ (Hybrid)</h4>` + resHtml;
        ...
        return `
            <div class="warehouse-card">
                ...
                <div style="display:flex; gap:0.5rem; margin-top:auto;">
                     ...
                     <button class="wh-action-btn" onclick="deleteFromWarehouse(${item.id})" style="background:#fee2e2; color:#ef4444;">?óë</button>
                </div>
            </div>
        `;
    }).join('');
...
window.addToWarehouse = (tempIdx) => {
    ...
    const nickname = prompt("?ÑÎûµ???âÎÑ§?ÑÏùÑ ?ÖÎ†•?òÏÑ∏??", "My Strategy " + (warehouseParams.length + 1));
    ...
    renderWarehouse();
    alert("?åÎùºÎØ∏ÌÑ∞ Ï∞ΩÍ≥†???Ä?•Îêò?àÏäµ?àÎã§.");
...
window.deleteFromWarehouse = (id) => {
    if (!confirm("?åÎùºÎØ∏ÌÑ∞ Ï∞ΩÍ≥†?êÏÑú ??†ú?òÏãúÍ≤†Ïäµ?àÍπå?")) return;
...
function renderSavedStrategies() {
    ...
    if (savedStrategies.length === 0) {
        if (containerDM) containerDM.innerHTML = `<div style="color:#64748b; font-style:italic;">?Ä?•Îêú ?åÎùºÎØ∏ÌÑ∞Í∞Ä ?ÜÏäµ?àÎã§.</div>`;
            ...
            <div class="strategy-card" style="background:white; border:1px solid #e2e8f0; border-radius:8px; padding:1rem; display:flex; flex-direction:column; gap:0.5rem; ${widthStyle}">
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <div style="display:flex;">
                         ...
                         <button onclick="deleteStrategy(${idx})" style="color:#ef4444; margin-right:5px; cursor:pointer; background:none; border:none; font-size:1rem;">?óë</button>
                    </div>
                </div>
...
window.saveCurrentParamsToWarehouse = () => {
    ...
    const nickname = prompt("?ÑÏû¨ ?åÎùºÎØ∏ÌÑ∞???âÎÑ§?ÑÏùÑ ?ÖÎ†•?òÏÑ∏??", "My Manual Strategy " + (warehouseParams.length + 1));
    ...
    localStorage.setItem('parameterWarehouse', JSON.stringify(warehouseParams));
    renderWarehouse();
    alert("?ÑÏû¨ ?åÎùºÎØ∏ÌÑ∞Í∞Ä Ï∞ΩÍ≥†???Ä?•Îêò?àÏäµ?àÎã§.");
...
window.deleteStrategy = (idx) => {
    if (!confirm("???ÑÎûµ????†ú?òÏãúÍ≤†Ïäµ?àÍπå?")) return;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (btnSave) {
        btnSave.addEventListener('click', () => {
            if (!currentDrillParams || !currentRobustnessStats) {
                alert("?ÑÏßÅ Î∂ÑÏÑù Í≤∞Í≥ºÍ∞Ä ?ÑÎ£å?òÏ? ?äÏïòÍ±∞ÎÇò ?ÜÏäµ?àÎã§.");
                return;
            }
    ...
    if (btnDeepDrill) {
        btnDeepDrill.addEventListener('click', async () => {
            if (!currentDrillParams) {
                alert("Î∂ÑÏÑù???åÎùºÎØ∏ÌÑ∞Í∞Ä ?ÜÏäµ?àÎã§.");
                return;
            }
            ...
            btnDeepDrill.textContent = "?íé DEEP DRILL (Apply & Run)";
        });
    }
});
